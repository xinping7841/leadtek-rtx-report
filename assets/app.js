(() => {
  ...
  function normalizeGpu(raw, price) {
  ...
  function renderLoadError(error) {
    if (!tableBody) return;
    tableBody.innerHTML = `<tr class="data-loading-row"><td colspan="17">GPU 数据加载失败：${escapeHtml(error.message)}</td></tr>`;
    if (hevcShelf) hevcShelf.innerHTML = `<div class="hevc-empty">GPU 数据加载失败，请检查 data/gpu-catalog.json 和 data/market-prices.json。</div>`;
  }

  function selectedGpus() {
    return gpus.filter((gpu) => selectedGpuIds.has(gpu.id));
  }
  ...
  function renderHevcShelf() {
    ...
    hevcShelf.innerHTML = items.map((gpu, idx) => {
  ...
  function updateState() {
    const selected = selectedGpus();
    compareCount.textContent = `已选 ${selected.length} 款`;
    buildCompare.disabled = selected.length < 2;
    clearCompare.disabled = selected.length === 0;
    toggleSelectedOnly.disabled = selected.length === 0;
    tableBody.querySelectorAll("tbody tr, tr").forEach((row) => {
      const gpuId = row.dataset.gpuId;
      if (!gpuId) return;
      const levelMatched = activeLevel === "all" || row.dataset.level === activeLevel;
      const checked = selectedGpuIds.has(gpuId);
      const input = row.querySelector(".compare-check");
      if (input) input.checked = checked;
      row.classList.toggle("is-selected", checked);
      row.classList.toggle("is-hidden-by-compare", selectedOnly && !checked);
  ...
  function buildComparisonTable() {
    ...
    compareOutput.innerHTML = `<div class="table-wrap"><table><thead><tr><th>项目</th>${header}</tr></thead><tbody>${body}</tbody></table></div>`;
    comparePanel.classList.add("is-visible");
    comparePanel.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function mergeExternalResults(payload) {
  ...
  async function loadExternalResults() {
    try {
      const payload = await loadJson(externalResultsUrl, { optional: true });
      if (!payload) {
      ...
      const matched = mergeExternalResults(payload);
  ...
  function toggleGpuSelection(gpuId) {
    if (!gpuId) return;
    if (selectedGpuIds.has(gpuId)) selectedGpuIds.delete(gpuId);
    else selectedGpuIds.add(gpuId);
    updateState();
  }
  ...
  function bindEvents() {
    table.addEventListener("change", (event) => {
      ...
      toggleGpuSelection(row?.dataset.gpuId);
    });
    ...
    [hevcSearch, hevcSort, hevcLevel, hevcArch, hevcMinNvdec, hevcMinMemory, hevcMaxPower, hevcNeed422, hevcNeedAv1, hevcLowPower, hevcHideWeak].forEach((control) => {
    ...
    hevcShelf?.addEventListener("click", (event) => {
      ...
      if (!button) return;
      toggleGpuSelection(button.dataset.gpuId);
    });
  ...
  async function init() {
    try {
      const [catalog, prices] = await Promise.all([loadJson(catalogUrl), loadJson(pricesUrl)]);
      const priceMap = new Map((prices.prices || []).map((price) => [price.gpuId, price]));
      gpus = (catalog.gpus || []).map((gpu) => normalizeGpu(gpu, priceMap.get(gpu.id)));
      renderTable();
      updateState();
      await loadExternalResults();
      window.setInterval(loadExternalResults, 60000);
    } catch (error) {
      renderLoadError(error);
      setSyncStatus("error", "GPU 数据加载失败", error.message);
    }
  }

  bindEvents();
  setViewMode(window.innerWidth < 1500 ? "compact" : "full");
  init();
})();
